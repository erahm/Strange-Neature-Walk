...
- There's a bug where if you log in as either a manager or admin, go to an exhibit and then log out, you'll still see the `edit` button and can enter edit mode. This is due to me not resetting the `editMode` variable but I decided to leave this bug in because it shows that the authz check through CASL is working by notifying the user that they lack the permissions to perform that action and then not allowing that action to go through.