...
export const categoryMutations = {
  createExhibitCategory: async (_, { name }, { prisma, ability }) => {
    if (!ability || !ability.can('create', 'ExhibitCategory')) {
      throw new Error('Insufficient permissions to create exhibit category', { code: 'FORBIDDEN', status: 403 });
    }
  ...
  updateExhibitCategory: async (_, { id, name }, { prisma, ability }) => {
    ...
    if (!ability || !ability.can('update', target)) {
      throw new Error('Insufficient permissions to update exhibit category', { code: 'FORBIDDEN', status: 403 });
    }
  ...
  deleteExhibitCategory: async (_, { id }, { prisma, ability }) => {
    ...
    if (!ability || !ability.can('delete', target)) {
      throw new Error('Insufficient permissions to delete exhibit category', { code: 'FORBIDDEN', status: 403 });
    }
